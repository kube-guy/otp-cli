...
권한 없이 단축키를 누르면 **타이핑 대신 클립보드에 복사**하고 경고음을 냅니다.
붙여넣기(⌘V)로 입력하면 됩니다. 아무 일도 일어나지 않아 고장으로 보이는 일은 없습니다.

> **`brew upgrade` 하면 권한을 다시 줘야 합니다.** 바이너리가 재빌드되면서 서명 해시가
> 바뀌어 기존 허용이 무효가 됩니다. 시스템 설정에서 기존 `otp` 항목을 **지우고**
> (토글만 끄지 말 것) 새 경로로 다시 추가한 뒤 `brew services restart otp` 하세요.
> 현재 경로는 `otp agent` 를 직접 실행하면 안내에 표시됩니다.

> 터미널에서 `otp agent` 를 직접 실행하면 권한이 있는 것처럼 보일 수 있습니다.
> macOS 가 권한을 '책임 프로세스'에 귀속시켜 터미널 앱의 권한을 물려받기 때문입니다.
> `brew services` 로 띄우면 `otp` 자신이 책임 프로세스가 되어 별도 허용이 필요합니다.
